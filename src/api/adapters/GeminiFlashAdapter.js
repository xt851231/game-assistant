...
export class GeminiFlashAdapter extends ModelAdapter {
    ...
    async connect(config) {
        ...
        try {
            ...
            this.tts = TTSFactory.createAdapter('gemini', this.config);
...
` });
            ...
            for await (const chunk of response) {
                ...
                if (fullText.includes("RESPONSE:")) {
                    ...
                    if (!transcriptionEmitted && transcription) {
                        console.log("🎤 Flash: Parsed Transcript:", transcription);
                        this.emit('content', {
    ...
    sendImage(base64Image, mimeType) {
        // For REST, we could accumulate images and send on next text call.
        // For this demo, we log a warning but don't throw.
        console.debug("GeminiFlashAdapter: sendImage received but ignored in real-time mode. Attach images to text messages instead.");
    }
