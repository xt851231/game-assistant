...
import { ShowAlertTool, AddCSSStyleTool } from '../lib/utils/tools';
...
export const LiveAPIProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const connect = async (config: AppConfig) => {
        ...
        try {
            // Re-create client
            clientRef.current = ModelClient.createAdapter(config.provider === 'gemini-live-websocket' ? 'live' : 'flash', {
                apiKey: config.apiKey,
                modelId: config.modelId,
                voice: config.voice,
                systemInstruction: config.systemInstructions,
                enableVAD: config.clientVAD,
                silenceDuration: config.silenceDuration,
                prefixPadding: config.prefixPadding,
                startSpeechSensitivity: config.startSpeechSensitivity === 'default' ? 'START_SENSITIVITY_UNSPECIFIED' : config.startSpeechSensitivity === 'high' ? 'START_SENSITIVITY_HIGH' : 'START_SENSITIVITY_LOW',
                endSpeechSensitivity: config.endSpeechSensitivity === 'default' ? 'END_SENSITIVITY_UNSPECIFIED' : config.endSpeechSensitivity === 'high' ? 'END_SENSITIVITY_HIGH' : 'END_SENSITIVITY_LOW',
            });
    ...
    const toggleAudio = async (enabled: boolean, micId: string, config: AppConfig) => {
        if (enabled) {
            ...
            if (audioStreamerRef.current) {
                audioStreamerRef.current.vadEnabled = config.clientVAD;
    ...
    const toggleVideo = async (enabled: boolean, camId: string, config: AppConfig) => {
        if (enabled) {
            ...
            if (videoStreamerRef.current) {
                ...
                if (videoStreamerRef.current) {
                    videoStreamerRef.current.alwaysTransmit = !config.clientVAD;
                }
    ...
    const toggleScreen = async (enabled: boolean, config: AppConfig) => {
        if (enabled) {
            ...
            if (screenCaptureRef.current) {
                ...
                if (screenCaptureRef.current) {
                    screenCaptureRef.current.alwaysTransmit = !config.clientVAD;
                }
